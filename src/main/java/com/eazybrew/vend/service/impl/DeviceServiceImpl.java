...
public class DeviceServiceImpl implements DeviceService {
    ...
    public DeviceResponse registerDevice(DeviceRequest request) {
        ...
        if (deviceRepository.existsByDeviceIdAndRecordStatus(request.getDeviceId(), RecordStatusConstant.ACTIVE)) {
            throw new CustomException("Device with ID " + request.getDeviceId() + " already exists", HttpStatus.BAD_REQUEST);
        }
        ...
        if (request.getVendingMachineId() != null && !request.getVendingMachineId().isEmpty() && 
                deviceRepository.existsByVendingMachineIdAndRecordStatus(request.getVendingMachineId(), RecordStatusConstant.ACTIVE)) {
            throw new CustomException("Device with Vending Machine ID " + request.getVendingMachineId() + " already exists", HttpStatus.BAD_REQUEST);
        }

        // Get company
        Company company = companyRepository.findByIdAndRecordStatus(request.getCompanyId(), RecordStatusConstant.ACTIVE)
        ...
        if (request.getVendingMachineName() != null && !request.getVendingMachineName().isEmpty()) {
            if(deviceRepository.existsByVendingMachineNameLikeIgnoreCaseAndCompanyAndRecordStatus(
                ...
                throw new CustomException("Device with Vending Machine Name " + request.getVendingMachineName() +
    ...
    public DeviceResponse getDeviceByVendingMachineId(String vendingMachineId) {
        Device device = deviceRepository.findByVendingMachineIdAndRecordStatus(vendingMachineId, RecordStatusConstant.ACTIVE)
                .orElseThrow(() -> new CustomException("Device not found", HttpStatus.NOT_FOUND));
    ...
    public DeviceResponse updateDevice(String deviceId, DeviceRequest request) {
        ...
        if (!device.getDeviceId().equals(request.getDeviceId()) &&
                deviceRepository.existsByDeviceIdAndRecordStatus(request.getDeviceId(), RecordStatusConstant.ACTIVE)) {
            throw new CustomException("Device with ID " + request.getDeviceId() + " already exists", HttpStatus.BAD_REQUEST);
        }

        // Check if vending machine ID is being changed and if it already exists
        if (request.getVendingMachineId() != null && !request.getVendingMachineId().isEmpty() &&
                (device.getVendingMachineId() == null || !device.getVendingMachineId().equals(request.getVendingMachineId())) &&
                deviceRepository.existsByVendingMachineIdAndRecordStatus(request.getVendingMachineId(), RecordStatusConstant.ACTIVE)) {
            throw new CustomException("Device with Vending Machine ID " + request.getVendingMachineId() + " already exists", HttpStatus.BAD_REQUEST);
        }
        ...
        if (!device.getCompany().getId().equals(request.getCompanyId())) {
            Company newCompany = companyRepository.findByIdAndRecordStatus(request.getCompanyId(), RecordStatusConstant.ACTIVE)
                    .orElseThrow(() -> new CustomException("Company not found", HttpStatus.NOT_FOUND));
